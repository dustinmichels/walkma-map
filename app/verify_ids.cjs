...
townsData.features.forEach((f) => {
  ...
  const name = (f.properties.CITY || f.properties.TOWN || '')
    .toUpperCase()
    .trim()
...
auditsData.features.forEach((f) => {
  ...
  if (matchedById + unmatched < 5) {
    console.log(
      `Audit Sample: Raw ID=${f.properties.TOWN_ID} (${typeOfOriginal}), Processed=${id}`
    )
  }
  ...
  if (id && townIds.has(id)) {
    ...
    const name = (f.properties['CITY/TOWN'] || f.properties.CITY || '')
      .toUpperCase()
      .trim()
